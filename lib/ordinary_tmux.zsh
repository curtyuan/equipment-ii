...
ii_zsh_tmux_alias_command() {
  ...
  print -r -- "ii=display-popup -EE -T 'ii pie ${version}' -w 90% -h 90% -d '#{pane_current_path}' ${(q)II_GO_ROOT}/script/ii-tmux-popup execute '#{pane_id}' '#{session_id}'"
}
...
ii_zsh_tmux_install_alias() {
  ...
  marker="version=${II_TMUX_INTEGRATION_SCHEMA} index=${alias_index} helper=${II_GO_ROOT}/script/ii-tmux-popup"
...
ii_zsh_tmux_ensure() {
  ...
  [[ -x "${II_GO_ROOT}/script/ii-tmux-popup" ]] || { print -u2 -- "ii: tmux popup helper is not executable: ${II_GO_ROOT}/script/ii-tmux-popup"; return 1; }
  ...
  expected_marker="version=${II_TMUX_INTEGRATION_SCHEMA} index=${marker_index} helper=${II_GO_ROOT}/script/ii-tmux-popup"
  ...
  if [[ -n "$alias_index" ]]; then
    ...
    notice="version=${II_TMUX_INTEGRATION_SCHEMA} helper=${II_GO_ROOT}/script/ii-tmux-popup"
...
ii_zsh_tmux_alias_state() {
  ...
  marker_index="$(ii_zsh_tmux_marker_field "$marker" index 2>/dev/null)"
  alias_index="${II_TMUX_ALIAS_INDEX[ii]:-}"
  expected="version=${II_TMUX_INTEGRATION_SCHEMA} index=${marker_index} helper=${II_GO_ROOT}/script/ii-tmux-popup"
...
ii_zsh_cmd_tmux() {
  ...
  print -rl -- "server: $server" "configured: $configured" "command alias: $(ii_zsh_tmux_alias_state "$marker")" \
    'command: ii' "helper: ${II_GO_ROOT}/script/ii-tmux-popup"
  if [[ "$binding" == *'if-shell -F "##{==:%1,ii pice}"'* && "$binding" == *ii-tmux-pice* ]]; then
