...
q or Ctrl-C aborts. Use ii load to load variables into this shell.
EOF
    return 0
  fi
  ...
  local key selected line name value plugin_file footer search_footer footer_status copy_rc normal_keys
    ...
    selected="$(
      ii_var_entries_for_fzf \
        | fzf -i --ansi --expect=enter --prompt='ii vars> ' --delimiter=$'\t' --with-nth=1,2,3 \
            ...
            --bind="esc:$(ii_fzf_modal_normal_actions "$footer" "$normal_keys")" \
            --bind='j:down,k:up,i:print(i)+accept,y:print(y)+accept,q:abort' \
            --preview="zsh -fc 'source \"\$1\"; printf \"%s\" \"\$2\" | ii_fzf_print_preview_with_footer \"\"' -- ${(q)plugin_file} {4..}" \
